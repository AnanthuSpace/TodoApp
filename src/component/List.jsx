...
function List({ index, content, todoArray, setTodoArray, complete, toast }) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if (result.value === todoArray[index].content) {
          toast("No changes found", "red"); 
        } else if (result.value.trim() === "") {
          toast("Enter valid content", "red");
        } else {
          const editedTodos = todoArray.map((elems, i) =>
            i === index ? { ...elems, content: result.value } : elems
          );
  ...
  const singleDelete = (index) => {
    const afterDelete = todoArray.filter((_, i) => i !== index);
  ...
  const completeTodo = (index) => {
    const afterCompleting = todoArray.map((elems, i) =>
      i === index ? { ...elems, complete: true } : elems
    );
    ...
    <div className="list-div">
      <div className="list-left">
        ...
        <div className="list-right">
          ...
          <button className="delete-btn" onClick={() => singleDelete(index)}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="15"
              width="15"
